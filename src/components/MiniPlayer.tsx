    ...
    <AnimatePresence>
      {track && (
        <motion.div
          ...
          className="fixed bottom-0 left-0 z-40 flex w-full items-center gap-4 border-t border-white/10 bg-black/90 px-4 py-3 text-white backdrop-blur-sm sm:px-8"
        >
          ...
          <div className="min-w-0 flex-1 sm:flex-none sm:w-48">
            ...
            <p className="truncate text-xs text-white/40">{album.title}</p>
          </div>

          <div className="hidden flex-1 items-center gap-3 sm:flex">
            <button onClick={prev} aria-label="Previous track" className="text-white/60 hover:text-white">
              ◀◀
            </button>
            <button
              onClick={togglePlay}
            ...
            <button onClick={next} aria-label="Next track" className="text-white/60 hover:text-white">
              ▶▶
            </button>
          ...
          <button onClick={stop} aria-label="Close player" className="shrink-0 text-white/40 hover:text-white">
            ✕
          </button>
        </motion.div>
      )}
